...
class PlayerPlane: SKSpriteNode {
    ...
    static func populate(at point: CGPoint) -> PlayerPlane {
        ...
        let atlas = Assets.shared.playerPlaneUpAtlas
        
        let playerPlaneTexture = atlas.textureNamed("airplane_3ver2_13") //SKTexture(imageNamed: "airplane_3ver2_13")
        //заменили инцилизатор SKSpiteNode т.к. PlayerPlane является наследником SKSpiteNode
        ...
        //физическое тело //// битовой маски BitMaskCategory
...
//              }
//        }
        let planeWaitAction = SKAction.wait(forDuration: 0.5)
...
//    }
    ...
    func handleTap(_ acceleration: CGFloat, x: CGFloat){
        
        playerX = x
        xAcceleration = acceleration //* 0.7 + self.xAcceleration * 0.3
    }
    
    func tapCheckPosition(){
        
        self.position.x += xAcceleration * 20
        
        if self.position.x > playerX && xAcceleration > 0 {
            self.position.x = playerX
        }else if self.position.x < playerX && xAcceleration < 0  {
            self.position.x = playerX
        }
    }
